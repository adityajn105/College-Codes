...
<h4 align="center" style="font:bold 30px Comic Sans MS;
        position:relative;color:#D04040;margin: 0px;padding: 0px;">Update GUEST INFO</h4>
<hr>
<%
    ...
    try {
        ...
        if( no!=null && name!=null && address!=null && noo!=null) {
            String queryInsert = "UPDATE GUESTS SET guest_no=?,name=?,address=? WHERE guest_no=?;";
            preparedStatement = con.prepareStatement(queryInsert);
            preparedStatement.setInt(1, Integer.parseInt(no));
            preparedStatement.setString(2, name);
            preparedStatement.setString(3, address);
            preparedStatement.setString(4,noo);
            int rows = preparedStatement.executeUpdate();
            out.print("<center>"+rows + " Rows Updated</center>");
        }
        else{
            if(noo!=null || no!=null || name!=null || address!=null){
%>
...
<center>
    <form action="update.jsp" method="post">
        <Label for="no">Guest No to be updated  :</Label>
        <input type="number" name="noo" id="noo" value="<%out.print(noo);%>">
        <br />
